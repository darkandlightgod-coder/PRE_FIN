...
def is_cloud_mode():
    return os.environ.get("GSPREAD_CREDENTIALS") is not None
...
def load_and_align_datasets():
    """
    💡 【多因子大合流黑科技】：
    自動從雲端 Google Sheets 拉取 Macro 因子、期權籌碼與輿情情緒，
    並以 Date 進行 Inner Join 精密時間戳記對齊，融合成 100+ 維度超強特徵空間！
    """
    ...
    print(f"   ✅ 三表合流完畢！最終多維特徵矩陣規模: {df.shape[0]} 天 | {df.shape[1]} 複合特徵變數。")
    return df
...
def execute_pca_and_prediction_pipeline(df, variance_threshold=0.85):
    ...
    df_predict_target = df.iloc[[-1]].reset_index(drop=True) # 以今日最新大數據特徵，預測明日
    ...
    print(f"   📈 [解釋變異數盤點]")
    print(f"     * 全域大合流特徵總數 (X): {len(feature_cols)} 個")
    print(f"     * 決策主成分數: 保留前 {optimal_k} 個正交成分 (累積解釋度 {cumulative_variance[optimal_k-1]*100:.2f}%)")
    ...
    print(f"\n🔗 【降維後正交 PC 分數與明日大盤回報相關性】")
    for pc, corr in correlations.items():
        print(f"   - {pc.ljust(10)} 相關係數: {corr:+.4f}")
    ...
    model = Ridge(alpha=15.0) # 提高正則化懲罰強度，防範大特徵過度擬合
    model.fit(X_tr, y_tr)
    ...
    print(f"\n📊 【量化預測模型回測驗證】")
    print(f"   - 測試天數: {test_size} 天 | 模擬測試集方向預測準確率: {accuracy:.2f}%")
...
def save_diagnostics_and_plots(pca_model, loadings, cumulative_variance):
    ...
    plt.close()
    print(f"🖼️ [本地存檔成功] 本地診斷分析圖表已生成: {DIAGNOSTICS_PLOT_PATH}")

def upload_plot_to_google_drive(file_path):
    folder_id = os.environ.get("GOOGLE_DRIVE_FOLDER_ID")
    creds_json = os.environ.get("GSPREAD_CREDENTIALS")
    if not folder_id or not creds_json:
        print("ℹ️ 未檢測到 GOOGLE_DRIVE_FOLDER_ID，跳過雲端圖表備份。")
        return
    try:
...
def sync_data_to_google_sheets(df_pca_features, prediction_report_dict):
    ...
    try:
        ...
        # 2. 覆蓋更新預測戰情表 (每次執行徹底清除並覆蓋最新一天的純數值預報)
...
def main():
    ...
    try:
        ...
        # 5. 組裝戰情報表 (純數值化)
        latest_close = df_pca_features.dropna(subset=["TWII_Close"]).iloc[-2]["TWII_Close"] # 今天大盤收盤
        predicted_target_close = latest_close * (1 + pred_return / 100)
        predicted_change_points = predicted_target_close - latest_close
        
        prediction_report = {
            "Date": df_pca_features.iloc[-1]["Date"], # 明日開盤預期日期
        ...
        # 6. 同步至 Google Drive & Google Sheets
